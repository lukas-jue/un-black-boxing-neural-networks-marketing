...
This network uses a random uniform distribution to initialize the weights for each neuron $H_i^l$.
...
An important feature of the \textit{best} model is that it does not memorize the relationships in the training data set---overfitting---
but rather learns the generalizable underlying mechanisms an thereby performs well on the holdout validation data set. 
For this purpose I first describe the optimization of the ANN through cross-validation of the predictions of several training epochs on a holdout 
...
For compiling the ANN, I use the adaptive moment estimation algorithm \code{Adam} \citep{kingmaAdamMethodStochastic2014}
