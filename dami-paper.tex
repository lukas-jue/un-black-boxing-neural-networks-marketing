...
The model with the highest \code{val\_acc} is then chosen to be the \textit{best} model for prediction for this data set.
...
Possible solutions to this problem is random search, which is even more efficient than grid search \citep{bergstraRandomSearchHyperparameter2012}.
...
the underlying mechanisms in the data, a neural network does not provide an easily interpretable function that links $Y$ to $X_i$.
